...
def execute_decoder_cnn(inputs, init_state, sequence_length, params, attention=None, cnn_rep=True, cnn_gen=False, mtype=4, use_batch_norm=False, 
                        dropout=0.5, dctype=5, all_pred=True, classify=False, init_input=None, grid_input=True):
    # push final state of encoder to decoder
    if params["fw_cell"] == "gru_block" or params["fw_cell"] == "rnn":
        dec_state = tf.squeeze(init_state[0], [0])
    else:
    ...
    for t in xrange(sequence_length):
        ...
        if not inputs is None and grid_input:
            input_t = inputs[:, t]
        ...
        # weather should be here?
        if not grid_input and not inputs is None:
            # concate decoder weather rep
            dec_in = tf.concat([dec_in, inputs[:, t]], axis=1, name="dec_we_rep")
        dec_out, dec_state = cell_dec(dec_in, dec_state)
        # perform generation steps
        # elmo connection, concate dec_in & lstm_hidden_output & encoder_attention
        if attention is not None: 
            dec_out = tf.concat([dec_out, attention], axis=1)
            if not grid_input:
                dec_out = tf.concat([dec_out, dec_in], axis=1, name="dec_elmo_connection")

        if all_pred or t == (sequence_length - 1):
            if cnn_gen:
                # standardize output to 256 dims
            ...
            if classify:
                class_val = tf.layers.dense(dec_out, 
                            params["districts"] * params["num_class"],
                            name="decoder_output_class",
...
def get_cnn_rep(cnn_inputs, mtype=4, activation=tf.nn.relu, max_filters=8, use_batch_norm=False, dropout=0.5, normalize=True):
    ...
    if mtype == 0:
        ...
        # 32 x 32 x 32 => 16x16x32 => 16x16x32 => 8 x 8 x 32 => 8x8x64 => 4x4x16
        ...
        # 32 x 32 x 32 => 16x16x32 => 8 x 8 x 32 => 4x4x16
        conv1 = get_cnn_unit(cnn_inputs, 32, (5,5), activation, "SAME", "rep_conv1", use_batch_norm, dropout)
        # 16x16x32 => 8x8x32
        ...
        # letnet-5
        # 32 x 32 x 8 => 16x16x16 
        conv1 = get_cnn_unit(cnn_inputs, 8, (5,5), activation, "SAME", "rep_conv1", use_batch_norm, dropout, strides=(1,1))
        conv1 = get_maxpool(conv1, (2,2), padding="VALID", name="rep_conv1_pool", strides=(2,2))
        
        # 16x16x16 => 8x8x32
        conv2 = get_cnn_unit(conv1, 32, (5,5), activation, "SAME", "rep_conv2", use_batch_norm, dropout, strides=(1,1))
        conv2 = get_maxpool(conv2, (2,2), padding="VALID", name="rep_conv2_pool", strides=(2,2))
        
        # 8x8x32 => 4x4x64
        conv3 = get_cnn_unit(conv2, 64, (5,5), activation, "SAME", "rep_conv3", use_batch_norm, dropout, strides=(1,1))
        conv3 = get_maxpool(conv3, (2,2), padding="VALID", name="rep_conv3_pool", strides=(2,2))
        
        # residual & multi-scale
        cnn_outputs2 = get_maxpool(conv1, (4,4), padding="VALID", name="final_conv1_pool", strides=(4,4))
        cnn_outputs3 = get_maxpool(conv2, (4,4), padding="VALID", name="final_conv2_pool", strides=(4,4))
        
        # flatten & concate
        cnn_outputs = tf.layers.flatten(conv3)
        cnn_outputs2 = tf.layers.flatten(cnn_outputs2)
        cnn_outputs3 = tf.layers.flatten(cnn_outputs3)
        cnn_outputs = tf.concat([cnn_outputs, cnn_outputs2, cnn_outputs3], axis=1)
        cnn_outputs = tf.layers.dense(cnn_outputs, 256, name="residual_multi_scale")
    else:
        # 25 x 25 x H => 11x11x32
        conv1 = get_cnn_unit(cnn_inputs, 32, (5,5), activation, "VALID", "rep_conv1", use_batch_norm, dropout)
        # 11x11x32 => 4x4x8 (32)
